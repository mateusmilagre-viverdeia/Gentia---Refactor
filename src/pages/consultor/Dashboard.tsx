...
import { Building2, Calendar, CheckCircle2, Clock, ArrowRight, AlertCircle } from "lucide-react";
...
export default function ConsultantDashboard() {
  ...
  const { isEPConsultant, isHeadCS, isSuperAdmin, loading: roleLoading } = useEPRole();
  ...
  // Redirect if not a consultant (Head CS and Super Admin go to their dashboards)
  if (!isEPConsultant && !isHeadCS && !isSuperAdmin) {
    return <Navigate to="/" replace />;
  }

  // Head CS and Super Admin go to Head CS dashboard
  if (isHeadCS || isSuperAdmin) {
    return <Navigate to="/head-cs/dashboard" replace />;
  }
