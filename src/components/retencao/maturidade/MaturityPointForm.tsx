...
interface MaturityPointFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  point?: TeamMaturityPoint | null;
...
export function MaturityPointForm({
  open,
  onOpenChange,
  point,
  position,
  ...
  const level = getMaturityLevelFromPosition(x, y);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-3 rounded-lg bg-muted/50 text-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              />
              <span className="font-medium">{config.level} - {config.label}</span>
            </div>
            <p className="text-muted-foreground text-xs mb-1">
              {config.sublabel}
            </p>
            <p className="text-muted-foreground text-xs">
              Estilo de liderança: {config.leadershipStyle}
            </p>
            <p className="text-muted-foreground text-xs">
              Decisão: {config.decisionStyle}
            </p>
          </div>
