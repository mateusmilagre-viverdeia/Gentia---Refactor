...
serve(async (req) => {
  ...
  try {
    ...
    if (!agentId) {
      const { data: step } = await supabase.from('recruitment_workflow_steps').select('agent_id').eq('job_id', session.job_id).eq('step_type', 'cultural').eq('is_active', true).maybeSingle();
      agentId = step?.agent_id || null;
    }
    if (!agentId) {
      const { data: agent } = await supabase.from('recruitment_agents').select('id').eq('account_id', accountId).eq('type', 'cultural').eq('is_active', true).limit(1).maybeSingle();
