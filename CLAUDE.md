...
- ✅ Front de PII fechado: marketplace (`07d1ff8`), careers (`a69ff69`) e **portal** (`b78a0a3` — 100% via `portal-data`, 0 acesso direto). ✅ Hardening RLS aplicado no destino (com OK do dono): **7 INSERTs anônimos removidos** + **leak de tokens** em `consultant_satisfaction_invites` fechado via RPC por token (`aa52c8d`/`c35ee8f`) + **execute de `anon` revogado em 69 funções definer privilegiadas** (add_credits/assign_role/admin_*; via REVOKE FROM PUBLIC, authenticated/service_role intactos; `750e8ce`). Tudo em `SECURITY_AUDIT §10.2`. ⏳ Resta (não-crítico, cutover): chrome-extension → destino; `post_reactions` SELECT público (baixa sev.).
