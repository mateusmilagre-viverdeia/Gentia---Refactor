...
- ✅ Front de PII fechado: marketplace (`07d1ff8`), careers (`a69ff69`) e **portal** (`b78a0a3` — 100% via `portal-data`, 0 acesso direto). ✅ Hardening RLS aplicado no destino (com OK do dono): **7 INSERTs anônimos removidos** + **leak de tokens** em `consultant_satisfaction_invites` fechado via RPC por token (`aa52c8d`/`c35ee8f`) + **execute de `anon` revogado em 69 funções definer privilegiadas** (add_credits/assign_role/admin_*; via REVOKE FROM PUBLIC, authenticated/service_role intactos; `750e8ce`). + `post_reactions` SELECT restrito a authenticated (`fcce6e6`). **Marco: 0 policies `qual/with_check=true` expostas a anon no schema.** Tudo em `SECURITY_AUDIT §10.2`. ⏳ Resta só (cutover): chrome-extension → destino.
...
*Última atualização: 2026-06-06 — **Dados reais carregados e VALIDADOS** no destino (59 empresas/181 candidatos/113 logs IA): integridade OK (RLS 416/416, 0 FKs órfãs), segurança advisor **0 ERROR**, **isolamento provado em contas reais**. Frentes: A ✅ · B ✅ · **F (LLMs)** 🟢 wrapper de desacoplamento implementado **atrás de flag** (default OFF) + Claude provado + parecer blindado (achado: 62% falha histórica nos modelos "pro", já resolvido) · **E (Infra)** 🟡 cache (`getConfiguredModel`) + pooling + fallback (`INFRA_ESCALA.md`) · **D (Backup)** 🟡 RPO/RTO + DR + restore doc (`BACKUP_DR.md`; PITR é decisão de custo). **Documentação 100% completa** (14 docs em `docs/`: SECURITY_AUDIT, PERFORMANCE_AUDIT, BANCO_DADOS, OBSERVABILITY, INFRA_ESCALA, BACKUP_DR, LLM_AUDIT, PLANO_EFICIENCIA_OPERACAO, SECRETS_INVENTORY, RUNBOOK_INCIDENTES, GUIA_BOAS_PRATICAS, HANDOFF, ENTREGA_CHECKLIST). **Todas as 6 frentes entregues** na parte viável pré-cutover. Falta p/ aceite só a **reunião de handoff** (com o cliente). Itens de **cutover** (go-live): ativar IA direta (key Gemini + flag), PITR, dados/secrets reais, reapontar crons/URLs, refs hardcoded, teste de restore. Fonte de verdade: `docs/ENTREGA_CHECKLIST.md` · plano de virada: `docs/HANDOFF.md §5`.*
