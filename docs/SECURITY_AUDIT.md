...
- **⏳ Pendente (cutover, junto da migração de storage):** `src/components/recruitment/marketplace/MarketplaceCandidateCard.tsx` e `MyUnlocksTab.tsx` consomem `revealed_data.cv_url` (vem da function `marketplace-unlock`, que repassa o `cv_url` do banco). Ajustar com `getSignedFileUrl` **ou** fazer `marketplace-unlock` emitir signed URL fresca. Fica para o cutover porque depende dos CVs reais migrados (paths/URLs definitivos). Nenhuma edge function usa `getPublicUrl` no bucket (não há quebra no backend).
