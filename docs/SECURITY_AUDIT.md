...
- **🔴 `anon` podia EXECUTAR 69 funções `SECURITY DEFINER` privilegiadas** (acesso via grant a `PUBLIC`, não a `anon` direto) — incl. `add_credits`/`consume_credits`/`refund_credits` (créditos), `assign_ep_role`/`assign_candidate_role`/`remove_ep_role` (papéis), `admin_*` (9 dashboards), `get_company_*_detail`, matching/search, filas de email. Correção: garantir grants explícitos a `authenticated`/`service_role` e `REVOKE EXECUTE FROM PUBLIC` (+ anon) → **anon=0** nas privilegiadas; **authenticated/service_role intactos** (9/9); mantidas RPCs por token, helpers de RLS e 10 RPCs públicas (NPS/portal/ROI/share/careers/get_public_job). Migration `20260606162000`; commit `750e8ce`. Advisor 0 ERROR.
- ✅ `post_reactions` SELECT restrito a `authenticated` (era `qual=true {public}` — anon lia reações; migration `20260606163000`, commit `fcce6e6`). `badges`/`survey_benchmarks` SELECT público = catálogos intencionais (OK).
- ✅ **Marco:** **0 policies com `qual/with_check=true` expostas a `anon`** em todo o schema (superfície permissiva anônima zerada).

## 10.3 Achados do cutover de refs / storage (2026-06-06)
- ⚠️→✅ **Regressão de bucket no import de storage:** a importação manual dos arquivos deixou **`candidate-files` público** (resetou o fix `20260602170000`), re-expondo CVs/avatares (PII). Re-aplicado **privado** (`20260606170000`); front já usa signed URLs. **Lição:** re-checar privacidade dos buckets após CADA import de storage (`RUNBOOK_CUTOVER` Fase 1).
- ✅ **`verify_jwt` triado e declarado na config** (resolve o mismatch): a `config.toml` agora declara **as 285 functions** → config = estado deployado → **redeploys seguros/duráveis** (não viram mais privadas por acidente). Triagem das 137 que faltavam:
  - **Público legítimo (`verify_jwt=false`):** webhook/stripe (4), intake externo (2), página pública/token (9), cron/agendado (12), entrevista/realtime (15) — validam via service_role / `x-cron-secret` / token de sessão.
  - **App autenticado (91): mantidos `false`** — espelham o deploy atual (que funciona) e têm validação interna (`_shared/auth-helpers`/service_role). Apertar p/ `true` exige teste (ver abaixo).
  - **Protegidas (`verify_jwt=true`, deploy v7):** `admin-financial-report` e `admin-manage-user` — verificadas como chamadas **só pelo front admin autenticado** (JWT do super_admin). `platform-admin-email`/`whatsapp` mantidas `false` (callers não localizados → não arriscar).
- ⏳ **Recomendação (cutover, com teste):** avaliar flipar p/ `verify_jwt=true` os ~91 "app autenticado" chamados **apenas por usuário logado** (o front envia o JWT do usuário). ⚠️ **NÃO flipar às cegas:** chamadas em contexto **anon** usam a *publishable key* (`sb_publishable_`, que **NÃO é JWT**) e quebrariam com `verify_jwt=true`; callers de cron/inter-função idem. Testar feature a feature.
- ✅ Storage no projeto novo validado: 13 buckets presentes; sensíveis (audio/docs/external-resumes) privados; `candidate-files` privado (corrigido). `avatars`/`culture-files`/assets públicos — revisar se a exposição é intencional.
