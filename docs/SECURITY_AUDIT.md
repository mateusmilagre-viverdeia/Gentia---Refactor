...
- ⚠️ **Mismatch `verify_jwt` (config × deployado):** **205 functions deployadas como públicas (`verify_jwt=false`)** mas só **65 declaradas** na `config.toml` → **140 públicas sem entrada**. Riscos: (a) redeploy sem `--no-verify-jwt` as tornaria privadas (quebra crawler/careers/webhooks/crons); (b) algumas podem estar públicas **sem intenção** (ex.: `admin-manage-user`, `admin-financial-report` — embora tenham checagem interna). **Auditar no cutover:** triar público-legítimo (declarar na config) vs. proteger (`verify_jwt=true`). Já declaradas: `sitemap`, `careers-public-data`, `careers-apply`.
